<script lang="ts">
	import type { QuestionType } from '$lib/types/entities';
	...
	const typeConfig: Record<QuestionType, { label: string; colors: string; icon: string }> = {
		'meta-research': {
			label: 'Research',
		...
		discussion: {
			label: 'Discussion',
			colors: 'bg-cosmic-cyan/20 text-cosmic-cyan',
