<script lang="ts">
	...
	let { consensus, divergentViewsData = null, itemName, blueprintUrl, blueprintExists = false, onOpenBlueprint }: Props = $props();
...
<div class="space-y-8">
	...
	<!-- Technical Blueprint -->
	{#if blueprintExists && onOpenBlueprint}
		<button onclick={onOpenBlueprint} class="w-full rounded-lg bg-space-700/50 border border-space-600 p-3 hover:border-cosmic-cyan/40 transition-all cursor-pointer group">
			<div class="flex items-center justify-between mb-2">
				<div class="flex items-center gap-2">
					...
					<span class="text-sm font-semibold text-star-white">Technical Blueprint</span>
				</div>
				<span class="text-xs text-star-faint group-hover:text-cosmic-cyan transition-colors flex items-center gap-1">
			...
			<img src={blueprintUrl} alt="Technical blueprint for {itemName}" class="w-full rounded border border-space-600 group-hover:border-cosmic-cyan/30 transition-all" />
		</button>
	{/if}
